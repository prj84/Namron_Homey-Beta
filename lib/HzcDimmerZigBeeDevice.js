...
class HzcDimmerZigBeeDevice extends ZigBeeDevice {
    ...
    levelControlCluster2() {
        const levelControlClusterEndpoint = this.getClusterEndpoint(CLUSTER.LEVEL_CONTROL);
        console.log("levelControlClusterEndpoint+++++++", levelControlClusterEndpoint)
    ...
    async registerDim() {
        if (this.hasCapability('dim')) {
            ...
            this.levelControlCluster().on('attr.currentLevel', async value => {
                ...
                const {onOff} = await this.onOffCluster().readAttributes(['onOff'])
                this.log('get onOff: ', onOff)
                if (!onOff) {
                    return
                }
